📬 email-triage-env
> A bilingual (English + Hinglish) reinforcement learning environment for email triage — built for the **Meta × Scaler School of Technology PyTorch Hackathon**.
![Python](https://img.shields.io/badge/python-3.10%2B-blue?style=flat-square)
![OpenEnv](https://img.shields.io/badge/OpenEnv-compatible-brightgreen?style=flat-square)
...
1	Email Analysis	Parse and understand the inbound email content
2	Category Classification	Route to one of: `urgent` · `billing` · `spam` · `support` · `info`
3	Priority Assignment	Assign one of: `high` · `medium` · `low`
Example Tasks
```python
# From tasks.py
...
Correct category OR priority (partial match)	`+0.5`
Incorrect classification	`0.0`
Exception / malformed output	`0.0`
...
1. Install
```bash
git clone https://github.com/Ujjawal0711/email-triage-env.git
cd email-triage-env
pip install -e .
```
2. Run with Docker
...
`uvicorn`	ASGI server
`pydantic`	Observation/Action schema validation
`openenv-core >= 0.2.0`	OpenEnv base classes and spec
